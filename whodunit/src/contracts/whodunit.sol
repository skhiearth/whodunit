...
contract WhoDunIt {
    ...
    function createContest(string memory _name, string memory _imei, string memory _description, uint _price, uint _entry, uint256 numberOfDays) public {
        Contest memory contest = Contest(contestCount, _imei, _name, _description, _price, _entry, (block.timestamp + (numberOfDays * 1 days)), payable(msg.sender)); // Create new instance of struct
        ...
        Contests[contestCount].deadline = block.timestamp + (numberOfDays * 1 days);
    ...
    function createBid(uint _id) public payable {
        ...
        Bid memory bid = Bid(bidCount, _id, msg.sender, Contests[_id].creator, Contests[_id].entry, Contests[_id].value); // Create new instance of struct
        UserBids[msg.sender].push(bid); // Add bid to list of users' bids
        
        PlacedBid memory placedBid = PlacedBid(bidCount, _id, msg.sender, Contests[_id].creator, Contests[_id].entry, Contests[_id].value); // Create new instance of struct
    ...
    function awardContest(uint _id) public {
        require(block.timestamp >= Contests[_id].deadline, "The contest can't be redeemed yet.");
    }
    
    // Return all bids placed by a bidder
    function getBidderBids(uint _index) public view returns (uint, uint, address, address, uint, uint) {
    ...
    function getCreatorBids(uint _index) public view returns (uint, uint, address, address, uint, uint) {
